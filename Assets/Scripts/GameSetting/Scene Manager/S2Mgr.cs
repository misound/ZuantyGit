...
public class S2Mgr : MonoBehaviour
{
    ...
    private void Enter()
    {
        ...
        for (int i = 1; i < 8; i++)
        {
            PlayerPrefs.SetString($"D2-{i}S", "false");
        }
        for (int i = 1; i < 5; i++)
        {
            PlayerPrefs.SetString($"AW2-{i}S", "false");
        }
        //抓元件
    ...
    public IList<Itemdata> CakeData1()
    {
        ...
        for (int i = 1; i < 9; i++)
        {
            result.Add(new Itemdata() { Name = $"D2-{i}", States = bool.Parse((PlayerPrefs.GetString($"D2-{i}S"))) });
        }
    ...
    public IList<AtkWData> CakeData2()
    {
        ...
        for (int i = 1; i < 5; i++)
        {
            result.Add(new AtkWData() { AWName = $"AW2-{i}", AWStates = bool.Parse((PlayerPrefs.GetString($"AW2-{i}S"))) });
        }
